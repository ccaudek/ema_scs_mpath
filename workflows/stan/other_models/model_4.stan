...
parameters {
  ...
  // Non-centered random intercepts for participants
  real<lower=0> sigma_participant_cs; // SD of participant intercepts for CS
  vector[P] z_participant; // Standard normal variables for non-centered parameterization
...
model {
  ...
  z_participant ~ normal(0, 1); // Priors for standard normal variables
  ...
  // Likelihood for CS
  for (n in 1:N) {
    real mu_cs = alpha_cs + beta_cs[1] * neg_affect[n] + beta_cs[2] * decentering[n] + beta_cs[3] * context_eval[n] + (sigma_participant_cs * z_participant[participant[n]]) + day_intercept_cs[participant[n], day[n]];
