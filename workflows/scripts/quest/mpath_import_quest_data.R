...
# "quest_ema_3_prep.csv" and "quest_ema_4_prep.csv".
...
# corresponding file.
file_names <- c("quest1.xlsx", "quest2.xlsx", "quest3.xlsx", "quest4.xlsx")
data_frames <- list()
...
for (i in seq_along(file_names)) {
  ...
  data <- data |> 
    dplyr::select(
      -c(nome_1, cognome_1, anno_1, mese_1, giorno_1, cellulare_1, sesso_1,
         nome, cognome, mese_c, giorno_c, cellulare_c, sex)
    )
...
quest_dat$Peso_alto_1 <- ifelse(
  quest_dat$Peso_alto_1 == "48,5", "48.5", quest_dat$Peso_alto_1 
)
quest_dat$Peso_alto_1 <- ifelse(
  quest_dat$Peso_alto_1 == "Boh", "63.85", quest_dat$Peso_alto_1 
)
quest_dat$weight_high <- 
  as.numeric(gsub(" kg", "", gsub("kg", "", quest_dat$Peso_alto_1)))
quest_dat$Peso_alto_1 <- NULL

quest_dat$Peso_basso_1 <- ifelse(
  quest_dat$Peso_basso_1 == "Boh", "56.5", quest_dat$Peso_basso_1 
)
quest_dat$weight_low <- 
  as.numeric(gsub(" kg", "", gsub("kg", "", quest_dat$Peso_basso_1)))
quest_dat$Peso_basso_1 <- NULL

quest_dat$Peso_ideale_1 <- ifelse(
  quest_dat$Peso_ideale_1 == "almeno 5 kg in meno", "63", quest_dat$Peso_ideale_1
)
quest_dat$Peso_ideale_1 <- ifelse(
  quest_dat$Peso_ideale_1 == "Il mio", "53", quest_dat$Peso_ideale_1
)
quest_dat$Peso_ideale_1 <- ifelse(
  quest_dat$Peso_ideale_1 == "53/54", "53.5", quest_dat$Peso_ideale_1
)
...
quest_dat$Peso_dieta_1 <- ifelse(
  quest_dat$Peso_dieta_1 == "4-5 kg", "4.5", quest_dat$Peso_dieta_1
)
quest_dat$Peso_dieta_1 <- ifelse(
  quest_dat$Peso_dieta_1 == "7/8 kili", "7.5", quest_dat$Peso_dieta_1
)

# Function to convert strings to numbers while handling non-numeric elements
convert_to_number <- function(x) {
  converted <- as.numeric(gsub("[^-0-9.]+", "", x))  # Remove non-numeric characters except "-" and "."
...
quest_dat$height <- as.numeric(gsub("[^0-9.]+", "", quest_dat$Altezza_1))
quest_dat$Altezza_1 <- NULL
quest_dat$height <- ifelse(quest_dat$height < 10, quest_dat$height*100, quest_dat$height)
...
rio::export(
  quest_dat,
  here::here("data", "prep", "quest_scales", "quest_tot.csv")
